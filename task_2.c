...
static struct rtimer lightSensor_timer, IMU_timer, pre_idle_timer;
static struct etimer state_timer;
static rtimer_clock_t lightSensor_timeout = RTIMER_SECOND / 4; // 4 Hz
static rtimer_clock_t IMU_timeout = RTIMER_SECOND / 20; // 20 HZ
...
static void do_IMU_timeout(struct rtimer *timer, void *ptr);
static void do_lightSensor_timeout(struct rtimer *timer, void *ptr);
...
PROCESS_THREAD(process_task_2, ev, data)
{
  ...
  while(1) {
    ...
    switch(currState) {
      case STATE_IDLE:
        rtimer_set(&lightSensor_timer, RTIMER_NOW() + lightSensor_timeout, 0, do_lightSensor_timeout, NULL);
        rtimer_set(&IMU_timer, RTIMER_NOW() + IMU_timeout, 0, do_IMU_timeout, NULL);
        PROCESS_YIELD();
        break;
      case STATE_BUZZ:
